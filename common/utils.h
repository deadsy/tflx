...
static inline float q31_to_float(int x) { return (float)x / (float)(1 << 31); }
...
// clamp x to >= a
static inline float clampf_lo(float x, float a) { return (x < a) ? a : x; }

// clamp x to <= a
static inline float clampf_hi(float x, float a) { return (x > a) ? a : x; }

//-----------------------------------------------------------------------------

// linear mapping map x = 0..1 to y = a..b
static inline float mapf(float x, float a, float b) { return ((b - a) * x) + a; }
...
static inline void reg_set(volatile uint32_t *reg, uint32_t bits) { reg_rmw(reg, bits, 0xffffffff); }

static inline void reg_clr(volatile uint32_t *reg, uint32_t bits) { reg_rmw(reg, bits, 0); }
